...
PLAIN_RULE = (
    "사람에게 남기는 결정/질문은 반드시 **쉬운 한 줄**로 써라: "
    "파일명·함수명·코드용어·계획 파일 이름을 쓰지 말고, '무엇을 정해야 하는지'와 "
    "'정하면 뭐가 되는지'만 일상어로. 바쁜 비전문가가 3초 만에 이해 못 하면 다시 써라.\n\n"
)

# 무엇을 사람에게 물을지의 경계. 해석·범위·접근법은 네가 정하고, 진짜 사람만 아는 것만 남겨라.
BLOCKER_RULE = (
    "사람에게 물어도 되는 건 **사람만 알 수 있는 것**뿐이다: 권한/접근키, 돈/예산, "
    "프로젝트 간 우선순위, 외부(타 조직)에 무엇을 요청할지, 되돌리기 어려운 선택. "
    "그 외 — 애매한 목표의 해석, 분석 범위, 기술 접근법, 무엇부터 볼지 — 는 **네가 정한다**. "
    "막히지 말고 가장 합리적인 쪽을 골라 '이렇게 가정했다'고 적고 진행하라. "
    "범위/해석을 사람에게 되묻는 질문은 블로커가 아니다.\n\n"
)

READONLY_REQUEST = (
    "지금은 분석만 한다(읽기 전용): 실제 파일을 읽어 현재 어디까지 왔는지 추정하고, "
    "'진전 기준'을 향해 다음 한 스텝을 제안하라. 경계를 넘지 말고, 사람만 정할 수 있는 "
    "결정이 있으면 재구성 질문으로 남겨라.\n\n" + PLAIN_RULE + BLOCKER_RULE +
