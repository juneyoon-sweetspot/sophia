...
# 진짜 claude(네이티브 설치)를 쓴다. command -v 는 세션 종속 래퍼(shim)를 가리킬 수 있어
...
CLAUDE_DIR="$(dirname "$CLAUDE")"   # launchd 깨끗 환경엔 claude 디렉터리가 PATH 에 없다 → 주입

# 핵심: launchd 의 로그인셸 PATH 엔 claude 가 없어서 워커/thinker/usage 가 전부 실패한다
# (전 라운드 $0 헛돎의 원인). claude 디렉터리를 PATH 앞에 박아 넣는다.
