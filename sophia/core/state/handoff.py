...
class Handoff:
    ...
    # 이 block episode 에 밑작업을 한 시점 mtime. blocked_mtime 과 같으면 '이미 밑작업함'
    # → 다음 라운드는 조용(또 안 파헤침). 사람이 건드려 새 block 이 되면 0 으로 리셋.
    groundwork_mtime: float = 0.0
    ...
    def _cap(self) -> None:
        ...
        if self.max_items and self.max_items > 0:
            ...
            self.commit_gates = self.commit_gates[-self.max_items:]
